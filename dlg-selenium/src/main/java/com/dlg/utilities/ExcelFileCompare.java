...
public class ExcelFileCompare {
    ...
    public static List<String> Comparator(String wbExpected, String wbActual) throws Exception{

        Workbook wb1 = WorkbookFactory.create(new FileInputStream(wbExpected));
        Workbook wb2 = WorkbookFactory.create(new FileInputStream(wbActual));
...
//        for (String d : ExcelFileCompare.compare(wb1, wb2)) {
//            System.out.println(d);
//        }

        return ExcelFileCompare.compare(wb1, wb2);

//        FileOutputStream outputStream = new FileOutputStream(new File("C:\\ExcelUtility\\testData023.xlsx"));
//        wb2.write(outputStream);
    }

    public static void Comparator(String wbExpected, String wbActual, String outputFileLocationAndName) throws Exception{

        Workbook wb1 = WorkbookFactory.create(new FileInputStream(wbExpected));
        Workbook wb2 = WorkbookFactory.create(new FileInputStream(wbActual));

        List<String> output = ExcelFileCompare.Comparator(wbExpected, wbActual);

        FileWriter writer = new FileWriter(outputFileLocationAndName);
    ...
    public static List<String> compare(Workbook wb1, Workbook wb2) {
        ...
        ExcelFileCompare excelComparator = new ExcelFileCompare();
        if (excelComparator.compareNumberOfSheets(loc1, loc2)) {
            if (excelComparator.compareSheetNames(loc1, loc2)) {
    ...
    private void compareDataInSheet(Locator loc1, Locator loc2) {
        int rowNum;
        if (loc1.sheet.getLastRowNum() >= loc2.sheet.getLastRowNum()) {
            rowNum = loc1.sheet.getLastRowNum();
        } else {
            rowNum = loc2.sheet.getLastRowNum();
        }
...
//            }
    ...
    private void compareDataInRow(Locator loc1, Locator loc2) {
        ...
        CellStyle Style = loc2.workbook.createCellStyle();
        loc2.workbook.createCellStyle();
        Style.setFillBackgroundColor(IndexedColors.GREY_50_PERCENT.getIndex());
        Style.setFillForegroundColor(IndexedColors.GREEN.getIndex());

        int cellNum;

        if (loc1.row.getLastCellNum() < loc2.row.getLastCellNum()) {
            cellNum = loc2.row.getLastCellNum();
        } else {
            cellNum = loc1.row.getLastCellNum();
        }
...
//            }
            ...
            if ((loc1.cell != null) && (loc2.cell == null)) {
                loc2.cell = loc2.row.createCell(k);
                loc2.cell.setCellValue("null");
//                continue;
            }

            if ((loc1.cell == null) && (loc2.cell != null)) {
                loc1.cell = loc1.row.createCell(k);
                loc1.cell.setCellValue("null");
//                continue;
            }
     ...
     */
    private boolean compareNumberOfSheets(Locator loc1, Locator loc2) {
     ...
     * Compare sheet names.
     */
    private boolean compareSheetNames(Locator loc1, Locator loc2) {
        boolean compStatus = true;
        for (int i = 0; i < loc1.workbook.getNumberOfSheets(); i++) {
    ...
    private void addMessageNull(Locator loc1, Locator loc2, String messageStart, String value1, String value2) {
        String str =
                String.format(Locale.ROOT, "%s\nworkbook1 -> %s -> %s [%s] != workbook2 -> %s -> %s [%s]",
                        messageStart,
                        loc1.sheet.getSheetName(), "test-loc1", value1,
                        loc2.sheet.getSheetName(), "test-loc2", value2
                );
        listOfDifferences.add(str);
    }
    ...
    private boolean isCellTypeMatches(Locator loc1, Locator loc2) {
        if (loc1.cell == null) {
            addMessageNull(loc1, loc2,
                    "Cell Data-Type does not Match in :: ",
                    "Actual is null", "Expected is not null");
            ;
            return false;
